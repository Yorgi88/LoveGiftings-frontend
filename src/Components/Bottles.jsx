...
const Bottles = () => {
  ...
  // fetch the prod using useEffect
  useEffect(()=>{
    dispatch(fetchProducts('bottles'))
  },[dispatch]);

  // optional but we should safe check we re-filter again just to make sure we get the filtered res
  // const bottleProd = products.filter((product)=> product.category.slug==='bottles');
const bottles = productsByCategory['bottles'] || [];
  if(isLoading){
    return (
      <LoadingIndicator/>
    );
  ...
  return (
    <section className="carousel-section" id="bottles">
      <div className="text-center mb-4">
        <h2>Bottles</h2>
        <p>Take a look at these slick bottles</p>
      </div>
      <Carousel
